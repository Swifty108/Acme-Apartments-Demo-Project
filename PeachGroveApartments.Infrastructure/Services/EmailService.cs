...
namespace PeachGroveApartments.Infrastructure.Services
{
    public class MailService : IMailService
    {
        ...
        public async Task SendEmailAsync(MailMessage mailRequest)
        {
            var email = new MimeMessage();
            //email.Sender = MailboxAddress.Parse(_mailSettings.Mail);
            //email.To.Add(MailboxAddress.Parse(mailRequest.ToEmail));
            //email.Subject = mailRequest.Subject;
            email.To.Add(new MailboxAddress("Raj N", "rajn10642@gmail.com"));
            email.From.Add(new MailboxAddress(mailRequest.SenderName, mailRequest.FromEmailAddress));

            //email.Sender = MailboxAddress.Parse(mailRequest.FromEmailAddress);
